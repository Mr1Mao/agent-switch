"""deepagents 中间件桥接：把 agent_core 的 hooks dispatcher 桥接到 deepagents 中间件协议。
...
（``adapter._build_agent``）延迟导入，``import agent_core`` 不会加载它。
"""
...
from agent_core.types import AgentConfig, AgentMessage, AgentResponse, ToolCall
...
class AgentHooksMiddleware(AgentMiddleware[Any, Any, Any]):
    """把 agent_core hooks dispatcher 桥接到 deepagents 的 agent 级与调用级事件。"""
...
def _state_messages(state: Any) -> list[AgentMessage]:
    """``state["messages"]``（LangChain 消息）→ agent_core 消息。"""
...
def _request_messages(request: Any) -> list[AgentMessage]:
    """``ModelRequest.messages``（LangChain 消息，不含 system）→ agent_core 消息。"""
    return [
...
def _request_tool_call(request: Any) -> ToolCall | None:
    """``ToolCallRequest.tool_call`` → agent_core ToolCall。"""
