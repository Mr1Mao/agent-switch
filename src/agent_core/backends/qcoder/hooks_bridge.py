"""agent_core hooks ↔ qoder-agent-sdk hooks 桥接。

把 agent_core 的 hooks dispatcher 编译成 ``QoderAgentOptions.hooks``
...
from agent_core.backends.qcoder.mapping import _import_sdk
...
#: agent_core 事件 → Qoder HookEvent 名（仅调用级，会话级由 adapter 层触发）
...
def _build_context(
    ...
    """把 Qoder hook_input（TypedDict）翻译成对应的 agent_core Context。"""
...
def _hook_result_to_output(event: AgentHookEvent, result: HookResult, hook_input: Any) -> dict[str, Any]:
    """把 agent_core HookResult 转成 Qoder SyncHookJSONOutput。"""
