"""qcoder 后端翻译层：agent_core 类型 ↔ qoder-agent-sdk 类型。

负责两部分：
1. **消息格式归一化**（双向）：agent_core ``AgentMessage`` ↔ qoder CLI wire 格式
...
def agent_messages_to_qoder_wire(messages: list[AgentMessage]) -> list[dict[str, Any]]:
    """agent_core 消息 → qoder CLI wire 格式消息列表（输入方向）。
...
def qoder_message_to_agent_message(message: Any) -> AgentMessage | None:
    """qoder SDK ``Message`` → agent_core 消息；无法归一的类型返回 None。
