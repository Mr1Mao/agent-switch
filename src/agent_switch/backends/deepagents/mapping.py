"""deepagents 适配器的翻译层：agent_switch 类型 ↔ LangChain / deepagents 类型。
...
from agent_switch.types.response import AgentChunk
...
#: LangChain 消息 type → agent_switch 角色
...
def import_create_deep_agent() -> Any:
    ...
    ``import agent_switch`` 本身不会加载 deepagents。
    """
...
def agent_messages_to_langchain(messages: list[AgentMessage]) -> list[Any]:
    """将 agent_switch 消息翻译为 LangChain 消息列表（输入方向）。
...
def langchain_message_to_agent_message(message: Any) -> AgentMessage:
    """LangChain 消息 → agent_switch 消息（输出方向）。
