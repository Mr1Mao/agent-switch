...
- 仅配置 ``agent_switch`` 命名空间 logger，``propagate=False``；
...
from agent_switch.types import AgentConfig, AgentMessage
from agent_switch.utils.input import format_input_preview

AGENT_SWITCH_LOGGER_NAME = "agent_switch"
...
def log_fields(**kwargs: Any) -> dict[str, Any]:
    """构造 logging ``extra=`` 使用的字段：``{"agent_switch": sanitized}``。

    Python logging 的 ``extra`` 只能带一层 key，这里把全部字段包进
    ``agent_switch`` 命名空间，Formatter 再从 ``record.agent_switch`` 读取，
    避免污染标准字段（如 message / levelname 等）。
    """
    return {"agent_switch": sanitize_dict(kwargs)}
...
def _extra_payload(record: logging.LogRecord) -> dict[str, Any]:
    """读取 ``record.agent_switch``（由 ``log_fields`` 写入），不存在时返回空 dict。"""
    payload = getattr(record, "agent_switch", None)
...
class AgentCoreDevFormatter(logging.Formatter):
    ...
    agent_switch 字段按 key 排序输出，保证同一事件的行格式稳定、易于 grep。
    """
...
class AgentCoreJsonFormatter(logging.Formatter):
    """JSON 格式：单行 ``{timestamp, severity, logger, message, ...agent_switch 字段}``。
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # agent_switch 字段平铺进顶层，与 timestamp/severity 并列
...
def configure_logging(level: int = logging.INFO, json: bool = False, stream: Any = sys.stderr) -> logging.Logger:
    """配置 ``agent_switch`` logger；重复调用会先清空已有 handler。

    :param level: 日志级别。
    :param json: True 使用 JSON Formatter，False 使用 Dev Formatter。
    :param stream: 输出流，默认 stderr。
    """
    logger = logging.getLogger(AGENT_SWITCH_LOGGER_NAME)
